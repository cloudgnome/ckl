class Menu{
	constructor(parameters){
		...
		this.init();
		$('#menu .load').on('click',this.load.bind(this));
	}
	load(event){
		var target = event.target;
		router.get(target.get('href'),target.get('view'),false,target.get('item-id'));
		target.addClass('active');

		if(this.activeMenuElement){
			this.activeMenuElement.removeClass('active');
		}
		this.activeMenuElement = target;
	...
	init(){
		this.activeMenuElement = $(`a.load[href="${location.pathname}"],a.load[model="${model}"]`);
