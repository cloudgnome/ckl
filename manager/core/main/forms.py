...
from main.models import Site,GoogleFeed,Storage,Percent,Price,Tigres,Meta
...
from catalog.models import City,Export
...
__all__ = ['PageDescriptionForm','CityDescriptionForm','CategoryDescriptionForm',
            ...
            'TagForm','RedirectForm','CurrencyForm','SeatForm'
        ]
...
class OrderForm(forms.ModelForm):
    ...
    name = forms.CharField(
            label='Имя',
            widget=forms.TextInput(
                attrs={'placeholder':'Имя','required':''}),
            required=False
        )
    lname = forms.CharField(
            label='Фамилия',
            widget=forms.TextInput(
                attrs={'placeholder':'Фамилия','required':''}),
            required=False
        )
    sname = forms.CharField(
            label='Отчество',
            widget=forms.TextInput(
                attrs={'placeholder':'Отчество','required':''}),
            required=False
        )
...
class SettingsForm(BetterModelForm):
    ...
    class Meta:
        ...
        fieldsets = [
                    ('main', 
                        {'fields':['logo','wotermark','attention_message'],
                        'legend':'Общее'}
                    ),
                    ('google', 
                        {'fields':['google_analytics','google_tag','google_verification'],
                        'legend':'Google'}
                    ),
                    ('novaposhta.ua',
                        {'fields':['api_key','phone'],
                        'legend':'novaposhta.ua'}
                    ),
                    ('contacts',
...
class DescriptionForm(BetterModelForm):
    ...
    def clean_meta_description(self):
        ...
        if self._meta.model.objects.filter(meta_description=meta_description,language__code=self.prefix).exclude(id=self.instance.id).exists():
    ...
    def clean_title(self):
        ...
        if self._meta.model.objects.filter(title=title,language__code=self.prefix).exclude(id=self.instance.id).exists():
    ...
    class Meta:
        fieldsets = [
                    ...
                    ('description', {'fields':['text'],'legend':'Описание','classes':['description']})
        ]
...
class ProductDescriptionForm(DescriptionForm):
    rozetka = forms.CharField(
            widget=RedactorEditor(allow_image_upload=False,
                attrs={"class":"description"}),
            label='',
            required=False
        )
    rozetka_name = forms.CharField(
            widget=forms.TextInput(
                attrs={'placeholder': 'Name','autocomplete':'off'}),
            label='Rozetka_Name:',
            required=False
    ...
    class Meta:
        fieldsets = [
                    ('main',{'fields':['name','title','meta_description','meta_keywords']}),
                    ('description', {'fields':['text'],'legend':'Описание','classes':['description']}),
                    ('rozetka',{'fields':['rozetka_name','rozetka'],'legend':'Rozetka','classes':['description']})
        ]
    ...
    class Meta:
        ...
        widgets = {
            ...
            'purchase_price': forms.TextInput(
                attrs={'placeholder':'Закуп.'}),
        }
        fieldsets = [
                    ('main', {'fields':['slug','model','is_available','storage','is_top'],'legend':'Общее'}),
                    ('prices', {'fields':['retail_price','big_opt_price','special','price_fixed','purchase_price','currency','counter'],'legend':'Цены','classes':['prices']}),
