        ...
        if obj and obj.get('cached') and os.path.isfile(path):
            with open(path,'rb') as content:
                if request.is_ajax() and page:
                    ...
                    response = render(request,'shop/%s/index.html' % request.folder,context={'view':slug.view,'content':content.read()})
                    response['Last-Modified'] = last_modified
                    return response
        elif obj:
            ...
            if request.is_ajax() and page:
                ...
                response = render(request,'shop/%s/index.html' % request.folder,context={'view':slug.view,'content':response.content})
                response['Last-Modified'] = last_modified
                return response
