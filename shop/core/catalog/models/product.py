...
class ProductDescription(Description):
    rozetka = RedactorField(null=True,verbose_name='Описание Rozetka.ua',
        redactor_options={'lang': 'ru'},allow_file_upload=False,allow_image_upload=False)
    rozetka_name = CharField(max_length=255,verbose_name='Имя Rozetka.ua',null=True)
...
class Product(Slugify):
    ...
    @property
    def rozetka_description(self):
        try:
            return self.description.get(rozetka_name__isnull=False,language__code='ua')
        except:
            return None
    ...
    @property
    def available(self):
        return '<div class="bool %s"></div>' % str(self.is_available).lower()
...
class Export(Model):
    product = ForeignKey(Product, related_name="export",on_delete=CASCADE)
    type = PositiveIntegerField(choices=export_choices,default=1)
    load = BooleanField(default=0)
